class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "hub-bot-#{conversation.inbox.id}",
      channel_id: "hub-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "hub-csml-bot-#{agent_bot.id}",
      name: "hub-csml-bot-#{agent_bot.id}",
      default_flow: 'hub_bot_flow',
      flows: [
        {
          id: "hub-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'hub_bot_flow',
