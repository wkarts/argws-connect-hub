...
describe Messages::Instagram::MessageBuilder do
  ...
  let!(:instagram_channel) { create(:channel_instagram_fb_page, account: account, instagram_id: 'hub-app-user-id-1') }
  ...
  describe '#perform' do
      ...
      allow(fb_object).to receive(:get_object).and_return(
        {
          ...
          profile_pic: 'https://hub-assets.local/sample.png'
    ...
    it 'discard echo message already sent by hub' do
      message
    ...
    it 'creates message with for reply with story id' do
      ...
      expect(message.content_attributes[:story_id]).to eq('hub-app-user-id-1')
      expect(message.content_attributes[:story_url]).to eq('https://hub-assets.local/sample.png')
    end
