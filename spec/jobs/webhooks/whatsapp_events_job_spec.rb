...
RSpec.describe Webhooks::WhatsappEventsJob do
  ...
  context 'when Connect|API provider' do
    ...
    let(:connect_params) do
      {
        object: 'whatsapp_business_account',
        phone_number: connect_channel.phone_number,
        entry: [{
          changes: [{
            value: {
              metadata: {
                phone_number_id: connect_channel.provider_config['phone_number_id'],
                display_phone_number: connect_channel.phone_number.delete('+')
              },
              messages: [{
                id: 'MAIN-FLOW-JOB-1',
                from: '557588449231',
    ...
    it 'uses the same status-aware service used by main' do
      ...
      described_class.perform_now(connect_params)

      expect(Whatsapp::IncomingMessageConnectApiStatusAwareService)
        .to have_received(:new)
        .with(inbox: connect_channel.inbox, params: connect_params)
