...
ActiveSupport::Notifications.subscribe('process_action.action_controller') do |_name, started, ended, _id, payload|
  ...
  attributes = {
    component: 'controller', event: payload[:action].to_s,
    account_id: params['account_id'].to_s.presence,
    http_status: payload[:status], duration_ms: ((ended - started) * 1000).round(2),
    details: { job_class: payload[:controller].to_s }
  }
  if error
    HubDiagnostics::Recorder.error('controller.failed', error, attributes.except(:event))
  else
    HubDiagnostics::Recorder.emit('controller.completed', attributes.except(:event))
  end
end
ActiveSupport::Notifications.subscribe('sql.active_record') do |_name, _started, _ended, _id, payload|
  ...
  HubDiagnostics::Recorder.emit('database.error', level: 'error', component: 'postgres',
...
if defined?(Sidekiq)
  Sidekiq.configure_server do |config|
    config.error_handlers << proc do |error, context, *_rest|
      ...
      HubDiagnostics::Recorder.error('sidekiq.error', error, component: 'sidekiq',
                                    job_id: job['jid'], job_class: job['class'], queue: job['queue'])
    end
  end
end
