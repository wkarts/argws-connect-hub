...
module TwoFactor
  class LoginChallenge
    PURPOSE = 'hub-frontend-two-factor-login'.freeze
    EXPIRES_IN = 5.minutes
    CONSUMED_PREFIX = 'hub:two_factor:consumed_challenge'.freeze
    ATTEMPTS_PREFIX = 'hub:two_factor:challenge_attempts'.freeze

    class << self
      def issue(user)
        verifier.generate(
          { user_id: user.id, nonce: SecureRandom.hex(16) },
      ...
      def verifier
        Rails.application.message_verifier(:hub_frontend_two_factor_login)
      end
    end
  end
end
