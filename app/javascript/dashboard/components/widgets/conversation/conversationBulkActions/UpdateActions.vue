...
const updateConversations = key => {
  if (key === 'snoozed') {
    ...
    commandPalette?.open({ parent: 'bulk_action_snooze_conversation' });
  } else {
    emit('update', key);
  }
};
...
<template>
  ...
  >
    ...
    <div class="px-2.5 pt-0 pb-2.5">
      <HubDropdownMenu class="m-0 list-none">
        <template v-for="action in actions">
          <HubDropdownItem v-if="showAction(action.key)" :key="action.key">
            <hub-button
              variant="clear"
              color-scheme="secondary"
              size="small"
