...
<template>
  ...
  >
    <ul class="mb-0 vertical dropdown menu">
      ...
      >
        <button
          class="flex group flex-col gap-0.5 overflow-hidden cursor-pointer items-start rounded-none py-2.5 px-2.5 justify-center w-full h-full text-left hover:bg-hub-50 dark:hover:bg-hub-800 border-x-0 border-t-0 border-b border-solid border-slate-100 dark:border-slate-700"
          :class="{
            ' bg-hub-25 dark:bg-hub-800': index === selectedIndex,
          }"
          @click="onListItemSelection(index)"
        >
          <p
            class="max-w-full min-w-0 mb-0 overflow-hidden text-sm font-medium text-slate-900 dark:text-slate-100 group-hover:text-hub-500 dark:group-hover:text-hub-500 text-ellipsis whitespace-nowrap"
            :class="{
              'text-hub-500 dark:text-hub-500': index === selectedIndex,
            }"
          >
            {{ item.description }}
          </p>
          <p
            class="max-w-full min-w-0 mb-0 overflow-hidden text-xs text-slate-500 dark:text-slate-300 group-hover:text-hub-500 dark:group-hover:text-hub-500 text-ellipsis whitespace-nowrap"
            :class="{
              'text-hub-500 dark:text-hub-500': index === selectedIndex,
            }"
          >
