...
const dayClasses = day => ({
  ...
  'text-slate-800 dark:text-slate-50 hover:text-slate-800 dark:hover:text-white hover:bg-hub-100 dark:hover:bg-hub-700':
    isInCurrentMonth(day),
  'bg-hub-600 dark:bg-hub-600 text-white dark:text-white':
    isSelectedStartOrEndDate(day) && isInCurrentMonth(day),
  'bg-hub-50 dark:bg-hub-800':
    (isInRange(day) || isHoveringInRange(day)) &&
    !isSelectedStartOrEndDate(day) &&
    isInCurrentMonth(day),
  'outline outline-1 outline-hub-200 -outline-offset-1 dark:outline-hub-700 text-hub-600 dark:text-hub-400':
      ...
      >
        ...
        <span
          v-if="
            ...
            isInCurrentMonth(day)
          "
          class="absolute bottom-0 w-6 h-8 ltr:-right-4 rtl:-left-4 bg-hub-50 dark:bg-hub-800 -z-10"
        />
      </div>
    </div>
  </div>
