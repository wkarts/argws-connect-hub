...
class DashboardAudioNotificationHelper {
  ...
  executeRecurringNotification = () => {
    if (!window.HUB || !window.HUB.$store) {
      this.clearSetTimeout();
      return;
    }
    const mineConversation = window.HUB.$store.getters.getMineChats({
      assigneeType: 'me',
      status: 'open',
    });
  ...
  isMessageFromCurrentConversation = message => {
    return (
      window.HUB.$store.getters.getSelectedChat?.id === message.conversation_id
    );
  };
