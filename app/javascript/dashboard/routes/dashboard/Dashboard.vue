...
import HubKeyShortcutModal from 'dashboard/components/widgets/modal/HubKeyShortcutModal.vue';
...
export default {
  ...
  watch: {
    displayLayoutType() {
      const { LAYOUT_TYPES } = hubConstants;
  ...
  methods: {
    handleResize() {
      const { SMALL_SCREEN_BREAKPOINT, LAYOUT_TYPES } = hubConstants;
      let throttled = false;
      const delay = 150;
...
<template>
  ...
  >
    ...
    <section class="flex flex-1 h-full min-h-0 px-0 overflow-hidden">
      ...
      />
      <hub-modal :show.sync="showAddLabelModal" :on-close="hideAddLabelPopup">
