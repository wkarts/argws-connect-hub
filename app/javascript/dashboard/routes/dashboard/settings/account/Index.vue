...
export default {
  ...
  computed: {
    ...
    hasAnUpdateAvailable() {
      if (!semver.valid(this.latestHubVersion)) {
        return false;
      }
...
<template>
  <div class="flex-grow flex-shrink min-w-0 p-6 overflow-auto">
    <form v-if="!uiFlags.isFetchingItem" @submit.prevent="updateAccount">
      ...
      <div class="p-4 text-sm text-center">
        ...
        <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
          {{
            $t('GENERAL_SETTINGS.UPDATE_HUB', {
              latestHubVersion: latestHubVersion,
            })
          }}
        </div>
        <div class="build-id">
    ...
    <hub-loading-state v-if="uiFlags.isFetchingItem" />
  </div>
</template>
