...
export default {
  ...
  computed: {
    ...
    isAHubInstance() {
      return this.globalConfig.installationName === 'Hub';
    },
  },
  beforeMount() {
    this.isLoading = this.isAHubInstance;
  },
  methods: {
    resizeContainers() {
      ...
      >
        <div class="px-8 max-w-[560px] w-full overflow-auto">
          <div class="mb-4">
            ...
            />
            <h2
              class="mt-6 text-3xl font-medium text-left mb-7 text-slate-900 dark:text-hub-50"
            >
              {{ $t('REGISTER.TRY_HUB') }}
            </h2>
          </div>
          <SignupForm />
          <div class="px-1 text-sm text-slate-800 dark:text-hub-50">
