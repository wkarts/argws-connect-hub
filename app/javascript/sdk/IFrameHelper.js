...
import { HUB_ERROR, HUB_READY } from '../widget/constants/sdkEvents';
...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'hub_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `hub-widget-holder hub--hide hub-elements--${window.$hub.position}`;
    if (window.$hub.hideMessageBubble) {
      holderClassName += ` hub-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$hub.widgetStyle)) {
      holderClassName += ` hub-widget-holder--flat`;
    }
  ...
  getAppFrame: () => document.getElementById('hub_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('hub--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `hub-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('hub-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('hub-widget:', ''));
  ...
  events: {
    loaded: message => {
      ...
      IFrameHelper.sendMessage('config-set', {
        ...
        showPopoutButton: window.$hub.showPopoutButton,
        widgetStyle: window.$hub.widgetStyle,
        darkMode: window.$hub.darkMode,
        showUnreadMessagesDialog: window.$hub.showUnreadMessagesDialog,
      ...
      if (window.$hub.user) {
        IFrameHelper.sendMessage('set-user', window.$hub.user);
      }
      ...
      if (!window.$hub.resetTriggered) {
        dispatchWindowEvent({ eventName: HUB_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: HUB_ERROR, data: data });
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$hub.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$hub.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$hub.baseDomain);
    },
    ...
    handleNotificationDot: event => {
      ...
      const bubbleElement = document.querySelector('.hub-widget-bubble');
      if (
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `hub_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$hub.hideMessageBubble);
    onLocationChangeListener();

    let className = 'hub-widget-bubble';
    let closeBtnClassName = `hub-elements--${window.$hub.position} hub-widget-bubble hub--close hub--hide`;

    if (isFlatWidgetStyle(window.$hub.widgetStyle)) {
      className += ' hub-widget-bubble--flat';
      closeBtnClassName += ' hub-widget-bubble--flat';
    }

    if (isWidgetColorLighter(widgetColor)) {
      className += ' hub-widget-bubble-color--lighter';
      closeBtnClassName += ' hub-widget-bubble-color--lighter';
    }
