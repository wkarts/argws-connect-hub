...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$hub.type)) {
    const textNode = document.getElementById('hub-widget--expanded__text');
    textNode.innerText = bubbleText;
  }
};

export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} hub-elements--${window.$hub.position}`;
  const bubbleIcon = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'svg'
  );
  bubbleIcon.setAttributeNS(null, 'id', 'hub-widget-bubble-icon');
  ...
  if (isExpandedView(window.$hub.type)) {
    ...
    textNode.id = 'hub-widget--expanded__text';
    textNode.innerText = '';
    target.appendChild(textNode);
    bubbleClassName += ' hub-widget--expanded';
  }
...
export const createBubbleHolder = hideMessageBubble => {
  if (hideMessageBubble) {
    addClasses(bubbleHolder, 'hub-hidden');
  }
  addClasses(bubbleHolder, 'hub--bubble-holder');
  bubbleHolder.style = 'overflow: hidden;';
  bubbleHolder.id = 'hub-bubble-holder';
  body.appendChild(bubbleHolder);
};

export const onBubbleClick = (props = {}) => {
  const { toggleValue } = props;
  const { isOpen } = window.$hub;
  if (isOpen !== toggleValue) {
    const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
    window.$hub.isOpen = newIsOpen;

    toggleClass(chatBubble, 'hub--hide');
    toggleClass(closeBubble, 'hub--hide');
    toggleClass(widgetHolder, 'hub--hide');
...
export const addUnreadClass = () => {
  const holderEl = document.querySelector('.hub-widget-holder');
