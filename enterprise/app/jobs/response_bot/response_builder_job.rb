class ResponseBot::ResponseBuilderJob < ApplicationJob
  ...
  def prepare_data(response_document)
    {
      ...
      messages: [
        ...
        { role: 'user', content: response_document.content }
      ]
    }
  end
  ...
  def system_message_content
    <<~SYSTEM_MESSAGE_CONTENT
      ...
      Ensure that you only generate faqs from the information provided in the message.
  ...
  def create_responses(response, response_document)
    ...
    JSON.parse(content.strip).fetch('faqs', []).each do |faq|
