module Enterprise::Concerns::Article
  ...
  included do
    ...
    def self.vector_search(params)
      ...
      records = joins(:category).search_by_category_slug(params[:category_slug]).search_by_category_locale(params[:locale]).search_by_author(params[:author_id]).search_by_status(params[:status])
