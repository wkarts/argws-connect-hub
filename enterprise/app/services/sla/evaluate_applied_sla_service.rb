class Sla::EvaluateAppliedSlaService
  ...
  def handle_missed_sla(applied_sla, type, meta = {})
    ...
    Rails.logger.warn "SLA #{type} missed for conversation #{applied_sla.conversation.id} in account #{applied_sla.account_id} for sla_policy #{applied_sla.sla_policy.id}"
  ...
  def handle_hit_sla(applied_sla)
    if applied_sla.active?
      ...
      Rails.logger.info "SLA hit for conversation #{applied_sla.conversation.id} in account #{applied_sla.account_id} for sla_policy #{applied_sla.sla_policy.id}"
    else
      applied_sla.update!(sla_status: 'missed')
      Rails.logger.info "SLA missed for conversation #{applied_sla.conversation.id} in account #{applied_sla.account_id} for sla_policy #{applied_sla.sla_policy.id}"
    end
  end
